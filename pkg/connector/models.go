...
// UserData struct holds raw data corresponding to a row in the 'users' tab.
...
// structure represents a single user definition before conversion to an SDK Resource object with a User trait.
...
// ResourceData struct holds raw data corresponding to a row in the 'resources' tab.
// It is defined for parsing data into an intermediary Go representation.
...
// structure represents a single resource definition before conversion to an SDK Resource object.
...
// EntitlementData struct holds raw data corresponding to a row in the 'entitlements' tab.
// It is defined for parsing data into an intermediary Go representation.
// It holds fields ResourceName (the resource it's defined on), Entitlement (acting as the slug), DisplayName, and Description.
// structure represents a single entitlement definition before conversion to an SDK Entitlement object.
type EntitlementData struct {
	...
	Entitlement  string `yaml:"entitlement" json:"entitlement"`     // acts as the Slug
	DisplayName  string `yaml:"display_name" json:"display_name"`
	Description  string `yaml:"description" json:"description"`
}

// GrantData struct holds raw data corresponding to a row in the 'grants' tab.
// It is defined for parsing data into an intermediary Go representation.
// It holds fields Principal (type:name[:membership_slug]) and EntitlementId (resource_name:entitlement_slug).
// structure represents a single grant relationship before conversion to an SDK Grant object.
...
// function is the constructor used by the main command to initialize the connector.
// main command requires this constructor to instantiate the connector server.
// Which provides the application entry point with a configured connector instance.
// implementation stores the provided file path for use during syncs.
